...
func (r Garden) MarshalJSON() ([]byte, error) {
	var concrete struct{}
	return json.Marshal(&concrete)
}

func (r *Garden) UnmarshalJSON(bs []byte) error {
	var concrete struct{}
...
func invoke(ctx context.Context, parentJSON []byte, parentName string, fnName string, inputArgs map[string][]byte) (_ any, err error) {
	...
	case "Garden":
		...
		case "MkdocsBaseContainer":
			...
			return (*Garden).MkdocsBaseContainer(&parent, ctx, dir), nil
		case "MkdocsService":
			var parent Garden
			err = json.Unmarshal(parentJSON, &parent)
			if err != nil {
			...
			return (*Garden).MkdocsService(&parent, ctx, dir), nil
		case "MkdocsBuild":
			var parent Garden
			err = json.Unmarshal(parentJSON, &parent)
			if err != nil {
			...
			return (*Garden).MkdocsBuild(&parent, ctx, dir), nil
		default:
			return nil, fmt.Errorf("unknown function %s", fnName)
		}
	case "":
		return dag.Module().
			WithDescription("A generated module for Garden functions\n\nThis module has been generated via dagger init and serves as a reference to\nbasic module structure as you get started with Dagger.\n\nTwo functions have been pre-created. You can modify, delete, or add to them,\nas needed. They demonstrate usage of arguments and return types using simple\necho and grep commands. The functions can be called from the dagger CLI or\nfrom one of the SDKs.\n\nThe first line in this comment block is a short description line and the\nrest is a long description with more detail on the module's purpose or usage,\nif appropriate. All modules should have a short description.\n").
			WithObject(
				dag.TypeDef().WithObject("Garden").
					WithFunction(
						dag.Function("MkdocsBaseContainer",
							dag.TypeDef().WithObject("Container")).
							WithDescription("MkdocsBaseContainer creates the docker image base container that other actions can be run from.").
							WithArg("dir", dag.TypeDef().WithObject("Directory"))).
					WithFunction(
						dag.Function("MkdocsService",
							dag.TypeDef().WithObject("Service")).
							WithDescription("MkdocsService starts and returns an HTTP service mounted to directory for live reload and preview of mkdocs.").
							WithArg("dir", dag.TypeDef().WithObject("Directory"))).
					WithFunction(
						dag.Function("MkdocsBuild",
